      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
           ...
           <div className="glass rounded-3xl p-6 border border-amber-500/30 bg-gradient-to-b from-amber-50 to-white dark:from-amber-900/20 dark:to-transparent">
              ...
              <p className="text-xs text-black dark:text-white mb-4">Never miss an urgent update. Enable SMS or WhatsApp alerts for school closures and exam notices.</p>
              <button className="w-full py-2 bg-amber-600 hover:bg-amber-500 rounded-xl text-white text-sm font-bold shadow-lg shadow-amber-500/20 transition-all">
                Manage Alerts
              </button>
           </div>
        ...
        <div className="lg:col-span-3">
           <div className="glass rounded-3xl p-6 border border-slate-200 dark:border-slate-700/50 min-h-[600px] bg-white dark:bg-transparent">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-0 mb-6 pb-4 border-b border-slate-200 dark:border-slate-800">
                 <h2 className="text-xl font-bold text-black dark:text-white flex items-center gap-2">
                   <span>📢</span> {filter === "All" ? "All Announcements" : `${filter} Announcements`}
                 </h2>
                 <button className="text-xs font-bold text-black dark:text-white hover:text-amber-600 dark:hover:text-amber-400 transition-colors flex items-center gap-1">
                   <span>✓</span> Mark All as Read
                 </button>
              </div>
              ...
              <div className="space-y-4">
                 {filteredAnnouncements.map((announcement) => (
                   <div 
                     key={announcement.id} 
                     className={`relative bg-slate-50 dark:bg-slate-900/60 p-6 rounded-2xl border transition-all hover:-translate-y-1 cursor-pointer ${
                       announcement.isUnread ? announcement.bg : "border-slate-200 dark:border-slate-700/50 hover:border-slate-400 dark:hover:border-slate-500"
                     }`}
                   >
                      {announcement.isUnread && (
                        <div className="absolute -top-1 -right-1 w-3 h-3 bg-amber-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(245,158,11,0.5)]"></div>
                      ...
                      <div className="flex items-start gap-4">
                         <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl shrink-0 bg-slate-100 dark:bg-slate-800 border ${announcement.isUnread ? announcement.color + " border-current" : "text-black dark:text-white border-slate-200 dark:border-slate-700"}`}>
                            {announcement.icon}
                         </div>
                         <div className="flex-1">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                               <div className="flex flex-wrap items-center gap-2">
                                  <span className={`text-[10px] uppercase font-black tracking-widest px-2 py-0.5 rounded border ${announcement.isUnread ? announcement.color + " border-current" : "text-black dark:text-white border-slate-300 dark:border-slate-600"}`}>
                                    {announcement.type}
                                  </span>
                            ...
                            <p className="text-sm text-black dark:text-white leading-relaxed mb-3">
                               {announcement.content}
                            </p>
                            
                            <div className="flex items-center gap-2 text-xs font-bold text-black dark:text-white">
                               <span>✍️ Posted by:</span>
                               <span className="text-black dark:text-white">{announcement.author}</span>
                           </div>
                         </div>
                      </div>
                   </div>
                 ...
                 {filteredAnnouncements.length === 0 && (
                    <div className="text-center py-20">
                       ...
                       <p className="text-black dark:text-white text-sm">No {filter.toLowerCase()} announcements right now.</p>
                    </div>
                 )}
              </div>
