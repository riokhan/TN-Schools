      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="glass rounded-3xl p-6 border border-slate-200 dark:border-slate-700/50 bg-white dark:bg-transparent">
              ...
              {myClubs.map((club, idx) => (
                <div key={idx} className="relative overflow-hidden rounded-2xl p-4 border border-slate-200 dark:border-slate-700/50 group cursor-pointer hover:border-slate-400 dark:hover:border-slate-500 transition-colors">
                  ...
                  <div className="relative z-10 flex items-start gap-4">
                    <div className="text-3xl">{club.icon}</div>
                    <div>
                      <h3 className="font-bold text-black dark:text-white mb-1">{club.name}</h3>
                      <span className="text-[10px] uppercase font-black tracking-wider px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded-md text-black dark:text-slate-300">{club.role}</span>
                      <p className="text-xs text-black dark:text-slate-400 mt-3 flex items-center gap-1">
                        <span>📅</span> {club.nextEvent}
                      </p>
                    </div>
                  </div>
            ...
            <Link href="/student/activities/directory" className="block text-center w-full mt-4 py-3 border border-dashed border-slate-300 dark:border-slate-600 rounded-xl text-sm font-bold text-black dark:text-slate-400 hover:text-purple-600 dark:hover:text-white hover:border-purple-400 dark:hover:border-slate-400 transition-colors">
              + Join another club
            </Link>
          </div>
            ...
            <ul className="space-y-0">
              {upcomingEvents.map((event, idx) => (
                <li key={idx} className="relative flex gap-4 pb-4">
                  ...
                  <div className={`w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center shrink-0 z-10 text-[10px]`}>
                    {event.icon}
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-black dark:text-white">{event.title}</h4>
                    <p className="text-xs text-black dark:text-slate-400 flex items-center gap-2 mt-1">
                      <span>{event.date}</span>
          ...
          {/* Activity Portfolio Shortcut */}
          <div className="glass rounded-3xl p-6 border border-purple-500/30 bg-gradient-to-br from-purple-50 to-white dark:from-purple-900/20 dark:to-transparent">
             <div className="flex items-center gap-4 mb-3">
               <div className="w-12 h-12 rounded-xl bg-purple-500/20 flex items-center justify-center text-2xl text-purple-600 dark:text-purple-400 border border-purple-500/30">
                 🏆
               </div>
               <div>
                 <h3 className="font-bold text-black dark:text-white">Activity Portfolio</h3>
                 <p className="text-xs text-black dark:text-slate-400">Track your extracurricular achievements.</p>
               </div>
             </div>
             <Link href="/student/middle-school/portfolio" className="block text-center w-full py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-sm font-bold transition-colors shadow-lg shadow-purple-500/20">
               View Portfolio
        ...
        {/* Right Column: Discover Clubs Grid */}
        <div className="lg:col-span-2">
          <div className="glass rounded-3xl p-6 border border-slate-200 dark:border-slate-700/50 min-h-full bg-white dark:bg-transparent">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
              ...
              <div className="relative">
                ...
                />
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-black dark:text-slate-500">🔎</span>
              </div>
            </div>
              ...
              {['all', 'arts', 'science', 'environment', 'academics', 'literature'].map((tab) => (
                <button 
                  ...
                  className={`px-4 py-1.5 rounded-full text-xs font-bold capitalize transition-colors ${
                    activeTab === tab 
                      ? 'bg-purple-500 text-white' 
                      : 'bg-slate-100 dark:bg-slate-800 text-black dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-purple-600 dark:hover:text-white'
                  }`}
                >
                  {tab}
                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredClubs.map((club, idx) => (
                <div key={idx} className={`rounded-2xl p-5 border ${club.bg} transition-all hover:-translate-y-1 cursor-pointer group`}>
                  <div className="flex justify-between items-start mb-4">
                     <div className="text-4xl bg-white dark:bg-slate-900/50 w-14 h-14 rounded-xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
                       {club.icon}
                     </div>
                     <span className={`text-[10px] font-black uppercase tracking-wider px-2 py-1 rounded-md ${club.tagBg} ${club.color}`}>
                       {club.category}
                  ...
                  <h3 className="text-lg font-bold text-black dark:text-white mb-1">{club.name}</h3>
                  <p className="text-xs text-black dark:text-slate-400 mb-4 flex items-center gap-1">
                    <span>👥</span> {club.members} Active Members
                  </p>
                  <Link 
                    href={`/student/activities/${club.name.toLowerCase().replace(" ", "-")}`}
              ...
              {filteredClubs.length === 0 && (
                <div className="col-span-full text-center py-12">
                   ...
                   <p className="text-sm text-black dark:text-slate-400">Try selecting a different category.</p>
                </div>
              )}
            </div>

            {/* Create Club Banner */}
            <div className="mt-8 rounded-2xl bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/40 dark:to-purple-900/40 border border-purple-500/30 p-6 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
               <div className="absolute top-0 right-0 w-32 h-32 bg-purple-500/20 blur-3xl rounded-full"></div>
               <div className="relative z-10">
                 <h3 className="text-lg font-bold text-black dark:text-white mb-1">Can't find what you're looking for?</h3>
                 <p className="text-sm text-black dark:text-slate-300">You can start your own club! Gather 5 students and a faculty sponsor to apply.</p>
               </div>
               <button className="relative z-10 shrink-0 px-6 py-3 bg-slate-900 dark:bg-white text-white dark:text-purple-900 rounded-xl font-bold shadow-lg hover:bg-slate-700 dark:hover:bg-slate-200 transition-colors">
                 Start a Club
               </button>
            </div>
