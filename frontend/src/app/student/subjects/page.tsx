...
export default function SubjectsPage() {
  return (
    ...
    >
          ...
          <div key={index} className="glass rounded-3xl p-6 border border-slate-200 dark:border-slate-700/50 hover:border-slate-400 dark:hover:border-slate-500/50 hover:-translate-y-1 transition-all group relative overflow-hidden flex flex-col h-full bg-white dark:bg-transparent">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6 relative z-10 mt-auto">
               <div className="bg-slate-50 dark:bg-slate-900/60 rounded-xl p-3 border border-slate-200 dark:border-slate-700/50 flex flex-col justify-center">
                  ...
                  <span className="text-sm font-semibold text-black dark:text-white">{subject.nextClass}</span>
               </div>
               <div className="bg-slate-50 dark:bg-slate-900/60 rounded-xl p-3 border border-slate-200 dark:border-slate-700/50 flex flex-col justify-center">
      ...
      {/* Recommended Focus Area */}
      <div className="glass rounded-3xl p-6 border border-amber-500/20 bg-gradient-to-br from-amber-50 to-white dark:from-amber-500/10 dark:to-transparent mb-6 flex flex-col md:flex-row items-center justify-between gap-6">
         <div className="flex items-center gap-5">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-3xl shadow-lg shrink-0">
               🎯
            </div>
            <div>
               <h3 className="text-lg font-bold text-black dark:text-white mb-1">Recommended Focus: Social Science</h3>
               <p className="text-sm text-black dark:text-white leading-relaxed">
                 You are slightly behind in Social Science (55% completed). You have a pending assignment due this Wednesday.
               </p>
            </div>
         </div>
         <button className="whitespace-nowrap px-6 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold rounded-xl hover:bg-slate-700 dark:hover:bg-slate-200 transition-colors shadow-lg active:scale-95">
            Start Learning Now
         </button>
      </div>
